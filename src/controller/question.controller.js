...
const fetchQuestions = async (req, res) => {
  try {
    // -----------------------------------------
    // ✅ STEP 1: TOKEN CHECK + FREE LIMIT LOGIC
    // -----------------------------------------
    ...
    // ✅ ADMIN / ADMIN-STUDENT = unlimited use
    if (!adminId && decoded.role !== "admin") {
      ...
      // ❌ FREE student → freeUsageCount check
    ...
    // ---------------------------------------------------
    // ⬇️ OLD FETCH LOGIC START (NO CHANGE IN THIS PART)
    // ---------------------------------------------------
    ...
    // Match chapter names to pdf tags
    for (const [subject, chapters] of Object.entries(dataofquestions)) {
      if (!subjectTargets[subject]) continue;
        ...
        const closestMatch = allPdfs
          ...
          .map((pdf) => ({
            ...
            distance: levenshtein.get(
              chapter,
              pdf.topic_tags.toLowerCase().trim()
            ),
          }))
          .sort((a, b) => a.distance - b.distance)[0];
      ...
      while (totalCollected < targetCount) {
        for (const { chapter, pdf_id } of chapterPdfPairs) {
          ...
          for (const question of selected) {
            ...
            questionsWithOptions.push({
              index: subjectIndexTracker[subject],
              question: {
                ...question.dataValues,
                subject,
                chapter,
        ...
        if (totalCollected < targetCount) break; // No more unique questions
      }
    }
